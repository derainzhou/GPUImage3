...
public class MetalRenderingDevice {
    ...
    init() {
        ...
        // Use Bundle(for:) to find the bundle containing this class, which is more robust
        // than Bundle.module if not building within a strict SPM context.
        guard let defaultLibrary = try? device.makeDefaultLibrary(bundle: Bundle(for: MetalRenderingDevice.self)) else {
